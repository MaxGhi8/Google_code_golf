...
def verify_program(task_path, examples):
...
if __name__ == "__main__":
    ...
    if arg.endswith(".py"):
        task_path = os.path.join(base_dir, arg) if not os.path.isabs(arg) else arg
        m = re.search(r"task(\d+)", os.path.basename(arg))
        task_num = int(m.group(1)) if m else 0
    else:
        task_num = int(arg)
        task_path = os.path.join(base_dir, f"task{task_num:03d}.py")
    examples = load_examples(task_num)
    verify_program(task_path, examples)