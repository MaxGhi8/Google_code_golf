...
curl -s $link \
  ...
  | tail -n 400 \
  | awk '{
      task = sprintf("%03d",NR); ftask = "task" task ".py";
      if (system("[ -f \"" ftask "\" ]") == 0) {
          cmd="wc -c < " ftask;
          cmd | getline size;
          close(cmd);
  ...
  | sort -nr | awk '$4<2500{print "task " $2 " could be " $1 "B smaller, they did in " $3 "B, we took " $4 "B"}; $4>=2500 {print "task " $2 " could be done in just " $3 "B, we should try it"}'
