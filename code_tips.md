...
11. To find the most frequent value of a matrix, I can use `max(L:=sum(g,[]),key=L.count)` with `g` the original matrix, see [task129.py](task129.py).
...
27. To coloring like gravity effect see [task322.py](task322.py) and [task032.py](task032.py) as reference. To see anti-gravity effect see [task128.py](task128.py).
...
70. Task 196, 251, 002, 279 are closely related, hope to improve them with a better logic in the future.